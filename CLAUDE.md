...
3. **Review:** spawn `mate-core:reviewer` (read-only) on the committed diff with the work item, the developer's report, and the path to sfmate's `rules/` directory. Fix findings conversationally; re-review the delta. The operator decides when it's done.
