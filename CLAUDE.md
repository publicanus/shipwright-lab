...
1. **Refine:** for new work, `sfmate:analyst` drafts a plain story on the GitHub issue — problem, 5–7 acceptance criteria as **rule + example** pairs, out-of-scope. Before the operator sees it, `sfmate:reviewer` (work-item mode) attacks the draft against the plugin's work-item rules — an untestable or existence-only criterion is CRITICAL and goes back to the analyst first. The operator edits or approves the reviewed body by reading it. No labels, no stages.
...
3. **Review:** spawn `sfmate:reviewer` (read-only) on the committed diff with the work item and the developer's report. Fix findings conversationally; re-review the delta. The operator decides when it's done.
